    ...
    return this.responseService.getRequest(endpoint).pipe(
      map((response: any) => {
          ...
          if (type_of_data === 'ProxmoxUser') {
            ...
            data = response.map((ldapEditProfileUser: any) => new LdapEditProfileUser(ldapEditProfileUser.givenName, ldapEditProfileUser.sn, ldapEditProfileUser.uid, this.displayService, ldapEditProfileUser.gidNumber, ldapEditProfileUser.wireguard));
            return data;
